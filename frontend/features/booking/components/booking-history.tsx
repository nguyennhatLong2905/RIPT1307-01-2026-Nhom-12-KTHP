...
export default function BookingHistory() {
  ...
  const handleCancel = async (id: number) => {
    try {
      ...
      alert(error.response?.data?.message || error.message || "Cannot cancel ticket. Please contact support.");
    }
  };

  if (isLoading) {
    return <div className="text-center py-20 text-[#c9a84c] animate-pulse">LOADING HISTORY...</div>;
  }
      ...
      {bookings.length === 0 ? (
        ...
        <div className="grid grid-cols-1 gap-4">
          {bookings.map(booking => {
            ...
            const isConfirming = deleteConfirmId === booking.id;

            return (
              <div
                key={booking.id}
                className={`bg-[#0d0d0d] border border-white/5 p-6 rounded-3xl transition-all duration-300 group relative overflow-hidden ${
                  isPast ? "opacity-75 grayscale-[0.5]" : "hover:border-[#c9a84c]/30"
                }`}
              >
                {!isPast && (
                    ...
                    <div className="flex gap-2 relative z-20">
                      ...
                      <div className="relative z-20">
                        <Button
                          variant="outline"
                          disabled={isPast}
                          onClick={e => { e.stopPropagation(); setDeleteConfirmId(isConfirming ? null : booking.id); }}
                        ...
                        {isConfirming && (
                          <div
                            className="absolute bottom-full left-1/2 -translate-x-1/2 mb-3 z-50 p-2 rounded-xl border flex items-center gap-2 shadow-2xl animate-in fade-in slide-in-from-bottom-2 duration-200"
                            style={{ background: "#1a1a1a", borderColor: "rgba(239,68,68,0.3)", backdropFilter: "blur(20px)", minWidth: "140px" }}
                          >

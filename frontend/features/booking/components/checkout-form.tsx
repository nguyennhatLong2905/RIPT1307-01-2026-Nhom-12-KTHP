...
export function CheckoutForm() {
  ...
  useEffect(() => {
    if (showtimeId) {
      showtimeService
        .getShowtimeById(parseInt(showtimeId))
        .then(setShowtime)
      ...
      <div className="w-full max-w-xl">
        ...
        <div className="mb-12">
          ...
          <div className="space-y-4">
            <div
              className={`border rounded-xl p-6 transition-all cursor-pointer ${isCard ? "border-[#c9a84c] bg-[#141414]" : "border-gray-800 bg-[#0a0a0a] hover:border-gray-600"}`}
              onClick={() => setPaymentMethod("card")}
            >
              <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-4">
                  <div className={`w-4 h-4 rounded-full border-[1.5px] flex items-center justify-center ${isCard ? "border-[#c9a84c]" : "border-gray-600"}`}>
                    {isCard && <div className="w-2 h-2 rounded-full bg-[#c9a84c]" />}
                  </div>
                  <span className={`text-[11px] font-bold tracking-wider ${isCard ? "text-[#c9a84c]" : "text-gray-400"}`}>
                    CREDIT/DEBIT CARD
                  </span>
                </div>
                <CreditCard className={`w-4 h-4 ${isCard ? "text-gray-400" : "text-gray-600"}`} />
              </div>
            ...
            <div
              className={`border rounded-xl p-6 transition-all cursor-pointer flex items-center justify-between ${isMomo ? "border-[#c9a84c] bg-[#141414]" : "border-gray-800 bg-[#0a0a0a] hover:border-gray-600"}`}
              onClick={() => setPaymentMethod("momo")}
            >
              <div className="flex items-center gap-4">
                <div className={`w-4 h-4 rounded-full border-[1.5px] flex items-center justify-center ${isMomo ? "border-[#c9a84c]" : "border-gray-600"}`}>
                  {isMomo && <div className="w-2 h-2 rounded-full bg-[#c9a84c]" />}
                </div>
                <span className={`text-[11px] font-bold tracking-wider ${isMomo ? "text-[#c9a84c]" : "text-gray-400"}`}>
                  MOMO E-WALLET
                </span>
              </div>

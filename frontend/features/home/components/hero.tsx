...
export default function Hero() {
  ...
  const handleBookNow = () => {
    if (heroMovie) router.push(`/movies/${heroMovie.id}`);
  };

  const handleToggleWishlist = async (e: React.MouseEvent) => {
    e.stopPropagation();
    if (!isLoggedIn()) { window.location.href = "/?login=true"; return; }
    if (!heroMovie) return;
    try {
      if (isLiked) {
    ...
    <div id="hero" className="w-full flex flex-col bg-black overflow-hidden">
      <div className="w-full min-h-[80vh] flex flex-col relative overflow-hidden">
        <div className="absolute inset-0 z-0 transition-opacity duration-1000" style={{ opacity: mounted ? 1 : 0 }}>
          {isVideo ? (
            <video autoPlay muted loop playsInline className="absolute inset-0 w-full h-full object-cover opacity-60" key={trailerUrl}>
              <source src={trailerUrl} type="video/mp4" />
            </video>
          ) : (
            <img src={heroMovie.posterUrl} alt={heroMovie.title} className="w-full h-full object-cover opacity-50" />
          )}
        </div>
        ...
        >
          <div className="flex flex-col gap-4 mb-5">
            ...
            <p className="text-[#E9C349] text-lg font-bold italic tracking-widest uppercase">{heroMovie.director}</p>
          </div>
          ...
          <div className="flex flex-col items-start gap-4 mb-8">
            ...
            <div className="flex items-center gap-2 bg-[#E9C349]/10 px-3 py-1.5 rounded-xl backdrop-blur-md border border-[#E9C349]/20">
              ...
              <span className="text-white text-md font-bold">
                9.0<span className="text-white/40 text-sm ml-1 font-normal">/ 10</span>
              </span>
            </div>
          </div>
